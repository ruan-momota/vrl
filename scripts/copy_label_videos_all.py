#!/usr/bin/env python3
"""Copy all videos listed by label split files.

The default layout matches the local SSV2-style sample:
...
Output videos are copied into one directory per data split:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Copy all videos referenced by label JSON files into "
            "split-specific directories."
        )
    )
    parser.add_argument(
        "--labels-dir",
        type=Path,
        default=Path("/labels"),
        help="Directory containing split JSON files. Default: /labels",
    )
    parser.add_argument(
        "--videos-dir",
        type=Path,
        default=Path("/videos"),
        help="Directory containing source videos named like <id>.<ext>. Default: /videos",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        required=True,
        help="Destination root. The script creates one subdirectory per data split.",
    )
    parser.add_argument(
        "--label-files",
        nargs="+",
...
def copy_split_videos(
    *,
    ...
    print(f"\nProcessing split '{split}' with {len(video_ids)} videos")
...
def main() -> int:
    ...
    for label_file in args.label_files:
        ...
        # Load every record from this split. No sampling or 100-item limit is applied.
        video_ids = load_video_ids(label_path)
        stats.append(
            copy_split_videos(
                split=split,
                video_ids=video_ids,
    ...
    for item in stats:
        ...
        total_copied += item.copied
        total_skipped_existing += item.skipped_existing
    ...
    print(
        f"total: requested={total_requested}, copied={total_copied}, "
        f"skipped_existing={total_skipped_existing}, missing={total_missing}"
    )
...
# Example:
# python3 scripts/copy_label_videos.py --output-dir /path/to/ssv2_all
