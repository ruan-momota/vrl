...
SSV2_CONFIG_DIR = Path("configs/runs/ssv2_dinov2_linear_probe")
SSV2_RUN_ID = "ssv2-c50-train100-heldout30-dinov2-base-frame-mean-frozen-linear-probe"
UCF101_CONFIG_DIR = Path("configs/runs/ucf101_dinov2_linear_probe")
UCF101_RUN_ID = "ucf101-c50-train100-heldout30-dinov2-base-frame-mean-frozen-linear-probe"
...
def test_ssv2_dinov2_extraction_configs_share_one_run_identity() -> None:
    ...
    assert {config.num_frames for config in configs} == {16}
    assert {config.image_size for config in configs} == {224}
...
def test_dinov2_evaluation_configs_match_the_heldout_artifacts() -> None:
    ...
    ucf101 = RunEvaluationConfig.from_file(
        UCF101_CONFIG_DIR / "ucf101_dinov2_c50_linear_probe_evaluation.json"
    )
