...
CONFIG_DIR = Path("configs/runs/hmdb51_slowfast_linear_probe")
RUN_ID = "hmdb51-full-split1-slowfast-r50-8x8-frozen-linear-probe"


def test_hmdb51_slowfast_extraction_configs_share_one_run_identity() -> None:
    configs = [
        RunConfig.from_file(path)
        for path in sorted(CONFIG_DIR.glob("hmdb51_slowfast_full_split1_*.json"))
    ...
    assert all(config.model_name == "slowfast" for config in configs)
    assert all(config.num_frames == 32 for config in configs)
    assert all(config.image_size == 256 for config in configs)


def test_hmdb51_slowfast_evaluation_config_matches_the_eight_heldout_artifacts() -> None:
    config = RunEvaluationConfig.from_file(
        CONFIG_DIR / "hmdb51_slowfast_full_split1_linear_probe_evaluation.json"
    )
