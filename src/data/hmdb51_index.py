"""HMDB51 directory-tree to normalized-subset-index conversion.

HMDB51 is distributed per-class as flat ``.avi`` archives with separate
official train/test split files. This builder expects the raw clips already
organized locally into ``<video_root>/<train|test>/<class>/<video>.avi``
(e.g. via the official ``testTrainMulti_7030_splits`` split files), the same
convention used by ``src.data.ucf101_index``: raw ``train`` videos become
``split="train"`` records and raw ``test`` videos become ``split="heldout"``
records (tagged with ``original_split="test"``). Output artifacts mirror the
other dataset cells:
...
def build_subset_index(
    *,
    ...
    """Build a normalized HMDB51 subset index from a train/test class tree."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build a normalized HMDB51 subset index from a train/test class-directory tree."
    )
    parser.add_argument(
        "--video-root",
        type=Path,
