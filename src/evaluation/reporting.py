"""Aggregate evaluation artifacts into report tables and plots."""
...
def run_reporting(
    *,
    ...
    report_basename: str = BASE_NAME,
    original_artifact_path: str | Path = DEFAULT_ORIGINAL_ARTIFACT,
    first_round_perturbations: set[str] | None = None,
    ...
    plots = Path(plot_dir)
    perturbations = first_round_perturbations or FIRST_ROUND_PERTURBATIONS
    reports.mkdir(parents=True, exist_ok=True)
    plots.mkdir(parents=True, exist_ok=True)

    baseline = _load_json(logs / f"{report_basename}_original_baseline_interpretability.json")
    sensitivity_logs = logs / "sensitivity"
    knn_logs = logs / "knn"
    sweep_logs = logs / "sweeps"
    sensitivity_summary = _load_json(
        sensitivity_logs / f"{report_basename}_all_perturbations_sensitivity_summary.json"
    )
    knn_drop_summary = _load_json(
        knn_logs / f"{report_basename}_all_perturbations_knn_drop_cosine.json"
    )
    class_report = _load_json(sensitivity_logs / f"{report_basename}_class_sensitivity.json")
    original_artifact = load_embedding_artifact(original_artifact_path)
    ...
    sweep_summaries = [
        _load_json(path)
        for path in sorted(sweep_logs.glob(f"{report_basename}_*_sweep_summary.json"))
    ...
    output_files = {
        ...
        "markdown_report": reports / f"{report_basename}_sensitivity_report.md",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--report-basename", default=BASE_NAME)
    parser.add_argument("--original-artifact", type=Path, default=DEFAULT_ORIGINAL_ARTIFACT)
...
def main() -> int:
    ...
    result = run_reporting(
        ...
        first_round_perturbations=None
        if args.perturbations is None
        else set(args.perturbations),
